...
function handleLoginSubmit(e) {
    ...
    fetch('/api/auth/login', {
        ...
        // Store auth token and user data using auth manager
        if (data.access_token && data.user) {
            const userData = {
                id: data.user.id,
                email: data.user.email,
                username: data.user.username,
                name: data.user.artist_name || data.user.username.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
            ...
            if (window.authManager) {
                window.authManager.login(userData, data.access_token);
            } else {
    ...
    fetch('/api/auth/register', {
        ...
        if (data.access_token && data.user) {
            const userData = {
                ...
                username: data.user.username,
                email: data.user.email,
                name: data.user.artist_name || `${firstName} ${lastName}`,
                is_artist: data.user.is_artist,
                newsletter: newsletter,
                signupTime: new Date().toISOString()
            };
            
            // Use auth manager to handle signup
            if (window.authManager) {
                window.authManager.login(userData, data.access_token);
            } else {
                // Fallback if auth manager not loaded
